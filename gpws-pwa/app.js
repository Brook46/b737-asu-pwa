...
(function () {
  ...
  // Vmo & minimum-manoeuvre / stick-shaker speeds (approx, config-dependent)
  const VMO = 340;
  function minMvr() { return state.flaps >= 30 ? 118 : state.flaps >= 15 ? 128 : state.flaps >= 5 ? 138 : 205; }
  function stallSpd() { return minMvr() - 18; }

  /* ================= mode definitions ================= */
  // act(level, aural, sound, lamps, nd) — lamps/nd optional
  function act(level, aural, sound, lamps, nd) {
    return { status: 'active', level, aural, sound, lamps: lamps || {}, nd: nd || {} };
  }
  ...
  const MODES = [
    {
      ...
      evaluate() {
        ...
        if (sink() > warning) return act('warning', 'WHOOP WHOOP — PULL UP', 'pull-up', { pullup: 'red' });
        if (sink() > caution) return act('caution', 'SINK RATE', 'sink-rate', { pullup: 'red' });
    ...
    },
    {
      ...
      evaluate() {
        ...
        const caution = 2000 + 1.2 * state.ra, warning = 3000 + 1.6 * state.ra;
        const landingCfg = state.gear === 'down' && landingFlaps();
        if (state.closure > warning && !landingCfg) return act('warning', 'TERRAIN TERRAIN — PULL UP', 'terrain-pull-up', { pullup: 'red' });
        if (state.closure > caution) return act('caution', 'TERRAIN TERRAIN', 'terrain', { pullup: 'red' });
    ...
    },
    {
      ...
      envelope: 'Gear up, RA < 500 ft. Low speed: “TOO LOW GEAR”; high speed: “TOO LOW TERRAIN”. Inhibited by GEAR INHIBIT. Red PULL UP on the attitude indicators.',
    ...
    },
    {
      ...
      envelope: 'Gear down, flaps not landing, RA < 245 ft. Low speed: “TOO LOW FLAPS”; high speed: “TOO LOW TERRAIN”. Inhibited by FLAP INHIBIT. Red PULL UP on the attitude indicators.',
    ...
    },
    {
      ...
      evaluate() {
        ...
        if (state.shear) return act('warning', 'WINDSHEAR, WINDSHEAR, WINDSHEAR', 'windshear',
          { windshear: 'red' }, { wsMsg: 'red', wsPfd: true });
    ...
    },
    {
      ...
      evaluate() {
        ...
        if (state.pws === 'caution')
          return act('caution', 'MONITOR RADAR DISPLAY', null, { windshear: 'amber' }, { wsMsg: 'amber', pwsSym: 'red' });
        return armed('armed — no windshear ahead on radar');
      }
    },
    {
      id: 'ta', name: 'EGPWS — Look-ahead terrain (TAD)',
      envelope: 'Terrain-database alerting. Caution 40–60 s: “CAUTION TERRAIN” + amber TERRAIN message & solid amber terrain on ND. Warning 20–30 s: “TERRAIN TERRAIN, PULL UP” + red PULL UP & solid red terrain. Inhibited by TERR INHIBIT.',
      demo: ['terrain', 'terrain-pull-up'], scn: { ra: 1600, vs: -1200, ias: 270, threat: 'warning', closure: 3000 },
      evaluate() {
        if (state.terrInhibit) return inh('TERR INHIBIT switch');
        if (state.threat === 'warning') return act('warning', 'TERRAIN TERRAIN — PULL UP', 'terrain-pull-up',
          { pullup: 'red', terr: 'red' }, { terrainMsg: 'red' });
        if (state.threat === 'caution') return act('caution', 'CAUTION TERRAIN', 'terrain',
          { terr: 'amber' }, { terrainMsg: 'amber' });
    ...
    },
    {
      ...
      evaluate() {
        if (state.ias > VMO) return act('warning', 'OVERSPEED (clacker)', 'overspeed', {});
        return armed(`armed — VMO ${VMO} kt`);
      }
    },
    {
      id: 'stall', name: 'Stall warning — stick shaker',
      envelope: 'Airspeed into the minimum-manoeuvre / stick-shaker region. Control-column stick shaker. Minimum-speed amber/red band shown on the PFD speed tape.',
      demo: ['stall'], scn: { ra: 800, vs: -500, ias: 120, phase: 'approach', gear: 'down', flaps: 30 },
      evaluate() {
        if (state.ias <= stallSpd()) return act('warning', 'STALL — STICK SHAKER', 'stall', {});
        return armed(`armed — shaker ~${Math.round(stallSpd())} kt in this config`);
      }
    }
  ];

  const PRIORITY = { m7: 100, stall: 99, ovsp: 98, pws: 95, m1: 90, ta: 85, m2: 84,
    m4a: 60, m4b: 59, m3: 55, m6: 40, m5: 30, tcf: 20 };
  ...
  function evaluateAll() {
    ...
    const nd = { terrainMsg: null, wsMsg: null, pwsSym: null, wsPfd: false };
    const active = [];
    for (const m of MODES) {
      ...
      if (r.status === 'active') {
        ...
        if (r.nd.terrainMsg) nd.terrainMsg = strongest(nd.terrainMsg, r.nd.terrainMsg);
        if (r.nd.wsMsg) nd.wsMsg = strongest(nd.wsMsg, r.nd.wsMsg);
        if (r.nd.pwsSym) nd.pwsSym = strongest(nd.pwsSym, r.nd.pwsSym);
  ...
  /* ================= approach callouts (FCOM 15.20.22) ================= */
  ...
  const GATE_SND = { 2500: 'alt-2500', 1000: 'alt-1000', 500: 'alt-500', 100: 'alt-100',
    50: 'alt-50', 40: 'alt-40', 30: 'alt-30', 20: 'alt-20', 10: 'alt-10' };
  let prevRa = state.ra;
  function calloutTick() {
    if (state.ra < prevRa && GpwsAudio.isEnabled() && !GpwsAudio.isBusy() && !state.testing) {
      for (const g of GATES) {
        if (prevRa > g && state.ra <= g) { setAuralText(GATE_TEXT[g]); GpwsAudio.play(GATE_SND[g]); break; }
      }
    }
    prevRa = state.ra;
  }
  ...
  for (const m of MODES) {
    ...
    el.innerHTML =
      `<div class="mode-head"><span class="mode-name">${m.name}</span><span class="chip" data-chip></span></div>
       <p class="mode-env">${m.envelope}</p>
       <div class="mode-foot"><span class="mode-reason" data-reason></span>
         <button class="play-btn" type="button" title="Play aural only">&#9654;</button></div>`;
  ...
  function renderLamps(lamps) {
    setLamp('lamp-pullup', lamps.pullup);
    setLamp('lamp-windshear', lamps.windshear);
    setLamp('lamp-belowgs', lamps.belowgs);
    setLamp('lamp-terr', lamps.terr);
    setLamp('lamp-inop', lamps.inop);
  }
  ...
  function line(x1, y1, x2, y2, cls) {
    ...
    if (cls) l.setAttribute('class', cls); return l;
  }
  ...
  (function ladder() { // pitch scale, 2.5° increments (FCOM 10.31.2)
    const g = $('pfd-ladder'), pxDeg = 4;
    for (let p = -20; p <= 20; p += 10) {
      if (p === 0) continue;
      const y = 130 - p * pxDeg, w = 18;
    ...
    for (let p = -25; p <= 25; p += 5) { if (p % 10 === 0) continue; const y = 130 - p * pxDeg; g.appendChild(line(130 - 8, y, 130 + 8, y, 'pfd-pitch')); }
    for (let p = -22.5; p <= 22.5; p += 5) { const y = 130 - p * pxDeg; g.appendChild(line(130 - 4, y, 130 + 4, y, 'pfd-pitch')); }
  })();
  const SPD_PX = 1.4;
  (function spdTape() {
    const g = $('pfd-spd-tape');
    for (let v = 90; v <= 400; v += 10) {
      const y = -v * SPD_PX;
      g.appendChild(line(46, y, 52, y, 'tape-tick'));
  ...
  (function altTape() {
    ...
    for (let v = 0; v <= 2500; v += 100) {
      const y = -v * ALT_PX;
      g.appendChild(line(210, y, 216, y, 'tape-tick'));
      if (v % 500 === 0) g.appendChild(text(218, y + 3, v, 'tape-num', 'start'));
    }
  })();
  (function compass() {
    const g = $('nd-compass'), cx = 130, cy = 250, R = 150;
    for (let a = -60; a <= 60; a += 10) {
      const rad = (a - 90) * Math.PI / 180;
      const x1 = cx + Math.cos(rad) * R, y1 = cy + Math.sin(rad) * R;
      const len = a % 30 === 0 ? 8 : 4;
      g.appendChild(line(x1, y1, cx + Math.cos(rad) * (R - len), cy + Math.sin(rad) * (R - len)));
    }
  })();

  // ND terrain speckle (FCOM 15.10.11 colour scheme)
  const ndGreen = $('nd-terr-green'), ndAmber = $('nd-terr-amber'), ndRed = $('nd-terr-red'), ndSolid = $('nd-terr-solid');
  (function seedTerrain() {
    const rnd = n => Math.random() * n;
    function speckle(group, count, color, band, r0) {
      let s = '';
      for (let i = 0; i < count; i++) {
        const a = -Math.PI * 0.83 + rnd(Math.PI * 0.66);
        const r = band[0] + rnd(band[1] - band[0]);
        const cx = 130 + Math.cos(a) * r, cy = 250 + Math.sin(a) * r;
        s += `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${(r0 + rnd(1.2)).toFixed(1)}" fill="${color}"/>`;
      }
      group.innerHTML = s;
    }
    speckle(ndGreen, 120, '#2f9e46', [40, 150], 0.8);  // dotted green — terrain below
    speckle(ndAmber, 85, '#e0a300', [30, 125], 1.0);   // dotted amber — near altitude
    speckle(ndRed, 65, '#e23b30', [20, 100], 1.1);     // dotted red — above altitude
  })();

  const pwsGroup = $('nd-pws');
  function pwsIconSvg() { // predictive windshear: red/black bands + amber radials (FCOM 15.10.14)
    let bands = '';
    for (let i = 0; i < 3; i++) {
      const yy = -8 + i * 8, col = i % 2 === 0 ? '#e23b30' : '#111';
      bands += `<path d="M-17 ${yy + 10} A22 15 0 0 1 17 ${yy + 10}" fill="none" stroke="${col}" stroke-width="4"/>`;
    }
    const radials = `<line x1="-20" y1="12" x2="-30" y2="20" stroke="#e0a300" stroke-width="1.5"/>
                     <line x1="20" y1="12" x2="30" y2="20" stroke="#e0a300" stroke-width="1.5"/>
                     <line x1="0" y1="-14" x2="0" y2="-24" stroke="#e0a300" stroke-width="1.5"/>`;
    return bands + radials + `<text x="0" y="30" text-anchor="middle" fill="#e23b30" font-family="Barlow Semi Condensed" font-weight="700" font-size="9">W/S</text>`;
  }

  /* ================= PFD / ND per-frame render ================= */
  const pfdWarn = $('pfd-warn'), ndTerrMsg = $('nd-terrain-msg'), ndWsMsg = $('nd-ws-msg'), ndTerrMode = $('nd-terr-mode');
  function barberPole(group, vLo, vHi, kind) { // draw striped band in tape coords
    let s = '';
    const step = 6;
    for (let v = vLo; v < vHi; v += step) {
      const y = -(v + step) * SPD_PX, h = step * SPD_PX;
      const col = kind === 'vmo' ? (Math.round(v / step) % 2 ? '#e23b30' : '#111')
                : kind === 'stall' ? (Math.round(v / step) % 2 ? '#e23b30' : '#111')
                : '#e0a300';
      s += `<rect x="46" y="${y.toFixed(1)}" width="7" height="${h.toFixed(1)}" fill="${col}"/>`;
    }
    group.innerHTML = s;
  }
  ...
  function renderDisplays(lamps, nd) {
    ...
    $('pfd-alt').textContent = state.ra;
    $('pfd-ra').textContent = state.ra;
    const mach = state.ias / 660; // crude IAS→Mach for display > 0.40
    $('pfd-mach').textContent = mach > 0.40 ? '.' + Math.round(mach * 100) : '';
    // VS digital (shown > 400 fpm) + pointer
    $('pfd-vs').textContent = Math.abs(state.vs) > 400 ? (state.vs > 0 ? '+' : '') + state.vs : '';
    $('pfd-vs').setAttribute('fill', state.vs < -1500 ? '#ff5147' : '#c9d3df');
    const vy = Math.max(70, Math.min(190, 130 - state.vs * 0.01));
    const vp = $('pfd-vs-ptr'); vp.setAttribute('y1', vy); vp.setAttribute('y2', vy);
    vp.setAttribute('stroke', state.vs < -1500 ? '#ff5147' : '#e8edf4');
    // tapes
    ...
    // Vmo / min-speed bands (move with the speed tape)
    const trans = `translate(0 ${130 + state.ias * SPD_PX})`;
    $('pfd-vmo').setAttribute('transform', trans);
    $('pfd-vmin').setAttribute('transform', trans);
    barberPole($('pfd-vmo'), VMO, 400, 'vmo');
    const mm = minMvr(), ss = stallSpd();
    barberPole($('pfd-vmin'), ss, mm, 'amber'); // amber min-manoeuvre band
    // stall red band below shaker speed appended
    ...
    $('pfd-gs-scale').setAttribute('stroke', lamps.belowgs ? '#ffb300' : '#c9d3df');
    dia.classList.toggle('pfd-flash', !!lamps.belowgs);
    $('pfd-gs').style.opacity = state.ils ? 1 : 0.35;
    // radio-alt box amber if very low (proxy for minimums)
    $('pfd-ra-box').setAttribute('stroke', state.ra < 50 ? '#ffb300' : '#0a0d12');
    $('pfd-ra').setAttribute('fill', state.ra < 50 ? '#ffb300' : '#e8edf4');
    // GPWS annunciation on PFD (red PULL UP / WINDSHEAR — FCOM 10.31.5)
    let w = '';
    if (lamps.pullup) w += '<g class="pfd-flash"><rect x="96" y="150" width="68" height="20" rx="2" fill="#d31a10"/><text x="130" y="165" text-anchor="middle" class="pfd-warn-t">PULL UP</text></g>';
    if (nd.wsPfd) w += '<g class="pfd-flash"><rect x="84" y="150" width="92" height="20" rx="2" fill="#d31a10"/><text x="130" y="165" text-anchor="middle" class="pfd-warn-t">WINDSHEAR</text></g>';
    pfdWarn.innerHTML = w;

    // ---- ND ----
    if (state.terrInhibit) {
      ndGreen.setAttribute('opacity', '0'); ndAmber.setAttribute('opacity', '0');
      ndRed.setAttribute('opacity', '0'); ndSolid.setAttribute('opacity', '0');
      ...
      // solid look-ahead patch ahead of own-ship
      if (nd.terrainMsg) {
        ndSolid.setAttribute('opacity', '1');
        ndSolid.innerHTML = `<ellipse cx="130" cy="150" rx="46" ry="34" fill="${nd.terrainMsg === 'red' ? '#e23b30' : '#e0a300'}"/>`;
      } else ndSolid.setAttribute('opacity', '0');
      ndTerrMode.textContent = 'TERR'; ndTerrMode.setAttribute('class', 'nd-mode');
    }
    // TERRAIN look-ahead message (all modes)
    ndTerrMsg.textContent = nd.terrainMsg ? (nd.terrainMsg === 'red' ? 'TERRAIN' : 'CAUTION TERRAIN') : '';
    ndTerrMsg.setAttribute('class', 'nd-msg ' + (nd.terrainMsg === 'red' ? 'msg-red' : 'msg-amber'));
    // WINDSHEAR message
    ndWsMsg.textContent = nd.wsMsg ? 'WINDSHEAR' : '';
    ndWsMsg.setAttribute('class', 'nd-msg ' + (nd.wsMsg === 'red' ? 'msg-red' : 'msg-amber'));
    // PWS symbol
    if (nd.pwsSym && !state.terrInhibit) {
      pwsGroup.setAttribute('opacity', '1'); pwsGroup.innerHTML = pwsIconSvg();
    } else { pwsGroup.setAttribute('opacity', '0'); }
  }
  ...
  function bindSlider(id, key) {
    ...
    inp.addEventListener('input', () => { state[key] = (id === 'gs') ? Number(inp.value) / 10 : Number(inp.value); out.textContent = fmt[key](state[key]); });
  ...
  bindSeg('seg-threat', 'threat'); bindSeg('seg-shear', 'shear', v => v === 'on'); bindSeg('seg-pws', 'pws');

  document.querySelectorAll('#panel-state input').forEach(el => el.addEventListener('input', () => { state.selectedMode = null; }));
  document.querySelectorAll('#panel-state .seg').forEach(el => el.addEventListener('click', () => { state.selectedMode = null; }));

  function bindSwitch(id, key) {
    const sw = $(id);
    sw.addEventListener('click', () => { state[key] = !state[key]; sw.classList.toggle('inhibit', state[key]); sw.setAttribute('aria-pressed', String(state[key])); });
  }
  ...
  function applyState(o) {
    ...
    state.tcfDemo = !!s.tcfDemo; prevRa = s.ra;
  }
  function selectMode(id) {
    ...
    if (disp.querySelector('.panel-title').getAttribute('aria-expanded') === 'false') disp.querySelector('.panel-title').click();
  }
  ...
  // SYS TEST (FCOM 15.10.8): lights + aural self-test sequence
  ...
  document.querySelector('.scenario-btns').addEventListener('click', e => {
    if (e.target.closest('button[data-scn="reset"]')) { state.selectedMode = null; for (const k in state.manual) state.manual[k] = false; applyState(CLEAR); }
  });
